...
RUN uv pip install --system \
    ...
    azul-plugin-dotnet-decompiler==$(cd /tmp/src && hatchling version)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    uv pip install --system --find-links /tmp/ azul-plugin-dotnet-decompiler==$(cd /tmp/src && hatchling version);fi
