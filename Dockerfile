...
RUN pip install \
    ...
    azul-plugin-dotnet-decompiler==$(cd /tmp/src && python -m setuptools_scm)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    pip install --find-links /tmp/ azul-plugin-dotnet-decompiler==$(cd /tmp/src && python -m setuptools_scm);fi
