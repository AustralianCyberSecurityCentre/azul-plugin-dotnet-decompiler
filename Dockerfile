...
RUN uv pip uninstall --system azul-plugin-dotnet-decompiler
RUN uv pip install --system --no-deps --find-links /tmp/ azul-plugin-dotnet-decompiler==$(hatchling version)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ]; then \
    uv pip freeze | grep 'azul-.*==' | grep -v '^azul-plugin-dotnet-decompiler' | cut -d "=" -f 1 | xargs -I {} uv pip install --extra-index-url=$UV_INDEX_URL --system --upgrade --no-deps --prerelease allow '{}>=0.0.0-dev'; \
    else \
    uv pip freeze | grep 'azul-.*==' | grep -v '^azul-plugin-dotnet-decompiler' | cut -d "=" -f 1 | xargs -I {} uv pip install --extra-index-url=$UV_INDEX_URL --system --upgrade --no-deps '{}>=0.0.0'; \
    fi
...
# Install dotnet tool needed by the plugin.
...
ENTRYPOINT ["azul-plugin-dotnet-decompiler"]